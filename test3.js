...
function findMatchingEntries(combinedData, MSData) {
    ...
    combinedData.forEach(item => {
        ...
        if (match && item.Translate !== match.Simp_TIMI) {
            const newObj = {
                'Label（key）': item.Key,
                'Simp_TIMI': match.Simp_TIMI, // F列
                'Simp_Chinese': match['Simp_Chinese'],
                'Simp_TIMI (Comment)': match['Simp_TIMI (Comment)'],
                'ms负责人': parsePersonInCharge(match['Simp_TIMI (Comment)']),
                '266文本': item.Translate, // combinedData的Translate
                'ms文本': match.Simp_TIMI, // 再次确认为F列
                '266备注': item.ToolRemark,
...
const worksheetData = comparisonResult.map(item => ({
    'Label（key）': item['Label（key）'],
    'Simp_TIMI': item['Simp_TIMI'],
    'Simp_Chinese': item['Simp_Chinese'],
    'Simp_TIMI (Comment)': item['Simp_TIMI (Comment)'],
    'ms负责人': item['ms负责人'],
    '266文本': item['266文本'],
    'ms文本': item['ms文本'],
    '266备注': item['266备注'],
